...
use biosim4_core::grid::visit_neighborhood;
...
pub struct CenterSparseChallenge {
    pub radius: f32,
...
impl Default for CenterSparseChallenge {
    fn default() -> Self {
        Self { radius: 0.25, min_neighbors: 5, max_neighbors: 8 }
    }
}
impl Challenge for CenterSparseChallenge {
    ...
    fn configure(&mut self, p: Value) -> Result<(), String> {
        ...
        if let Some(v) = p.get("min_neighbors") {
            self.min_neighbors = v.as_u64().ok_or("min_neighbors")? as u32;
        }
        if let Some(v) = p.get("max_neighbors") {
            self.max_neighbors = v.as_u64().ok_or("max_neighbors")? as u32;
        }
        Ok(())
    }
    fn evaluate(&self, agent: &Agent, world: &World) -> (bool, f32) {
        ...
        if (dx * dx + dy * dy).sqrt() < self.radius / 2.0 {
            return (false, 0.0);
        }
...
pub struct StringChallenge {
    pub min_neighbors: u32,
    pub max_neighbors: u32,
}
impl Default for StringChallenge {
    fn default() -> Self {
        Self { min_neighbors: 2, max_neighbors: 2 }
    }
}
...
fn neighbor_count(loc: biosim4_core::types::Coord, radius: f32, world: &World) -> u32 {
    let mut count = 0u32;
    visit_neighborhood(world.grid, loc, radius, |nloc| {
        if nloc != loc && world.grid.is_occupied_at(nloc) {
            count += 1;
        }
    });
    count
