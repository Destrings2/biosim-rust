...
/// Per-cell food values in [0.0, 1.0]. Flat row-major layout:
/// `cells[y * size_x + x]`.
...
impl FoodLayer {
    ...
    pub fn new(size_x: u16, size_y: u16) -> Self {
        let cells = vec![0.0f32; size_x as usize * size_y as usize];
    ...
    pub fn randomize(&mut self, density: f32, grid: &Grid, rng: &mut Rng) {
        for y in 0..self.size_y as usize {
            for x in 0..self.size_x as usize {
                ...
                let i = y * self.size_x as usize + x;
                if grid.is_barrier_at(loc) {
                    self.cells[i] = 0.0;
                } else {
                    self.cells[i] = if rng.gen_f32() < density { 1.0 } else { 0.0 };
                }
            }
        }
    }
    ...
    pub fn regenerate(&mut self, rate: f32, grid: &Grid) {
        for y in 0..self.size_y as usize {
            for x in 0..self.size_x as usize {
                ...
                if !grid.is_barrier_at(loc) {
                    let i = y * self.size_x as usize + x;
                    self.cells[i] = (self.cells[i] + rate).min(1.0);
                }
            }
        }
    }
