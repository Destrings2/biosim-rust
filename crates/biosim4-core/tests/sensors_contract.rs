...
fn every_sensor_returns_in_unit_interval() {
    ...
    let mut ctx =
        SensorContext { agent: agent_ref, world: &world, sim_step: 5, rng: &mut sensor_rng };
...
fn loc_x_sensor_at_extremes() {
    ...
    let v_left = reg.evaluate(
        loc_x_idx,
        &mut SensorContext {
            agent: population.get(1).unwrap(),
            world: &world,
            sim_step: 0,
            rng: &mut srng,
        },
    );
    let v_right = reg.evaluate(
        loc_x_idx,
        &mut SensorContext {
            agent: population.get(2).unwrap(),
            world: &world,
            sim_step: 0,
            rng: &mut srng,
        },
...
fn boundary_dist_at_corner_is_zero_at_center_is_one() {
    ...
    assert!(
        (v_center - 1.0).abs() < 1e-6,
        "boundary_dist at center should be 1.0 (got {})",
        v_center
    );
}
...
fn osc1_oscillates_with_step() {
    ...
    let mut srng = Rng::seeded(0);
    let agent_ref = population.get(1).unwrap();
    let v_at_0 = reg.evaluate(
        osc_idx,
        &mut SensorContext { agent: agent_ref, world: &world, sim_step: 0, rng: &mut srng },
    );
    let v_at_8 = reg.evaluate(
        osc_idx,
        &mut SensorContext { agent: agent_ref, world: &world, sim_step: 8, rng: &mut srng },
    );
    let v_at_16 = reg.evaluate(
        osc_idx,
...
fn random_sensor_produces_varied_output() {
    ...
    let samples: Vec<f32> = (0..50)
        .map(|s| {
            reg.evaluate(
                r_idx,
                &mut SensorContext { agent: agent_ref, world: &world, sim_step: s, rng: &mut srng },
            )
        })
        .collect();
